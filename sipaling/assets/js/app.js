...
// 🖱️ Soft pop when player casts a vote
...
function createRoom() {
    ...
    if (!nameInput || !isValidName(nameInput)) { alert('Nama hanya boleh huruf (A-Z), tanpa angka/spasi, max 12 huruf!'); return; }
    ...
    db.ref('rooms').once('value', snapshot => {
        ...
        if (totalRooms >= 2) {
            alert('Jumlah room aktif sudah mencapai batas maksimum (2). Coba lagi nanti.');
            return;
        }
        myName = nameInput;
...
function joinRoom() {
    ...
    db.ref('rooms/' + codeInput).once('value', snapshot => {
        ...
        if (!snapshot.exists()) { alert('Room tidak ditemukan!'); return; }
        const data = snapshot.val();
        
        const playerCount = data.players ? Object.keys(data.players).length : 0;
        if (playerCount >= 25) { alert('Room sudah penuh! Maksimal 25 pemain.'); return; }
...
function triggerShuffle() {
    ...
    if (!availableDeck.length) {
        alert("Semua kartu di kategori ini sudah dimainkan! Mengacak ulang deck.");
        availableDeck = deck;
        usedCards = [];
    }
...
function submitVote(targetPlayerId) {
    ...
    if (myPlayerData && myPlayerData.joinedMidGame) {
        showInfoToast('⏳ Kamu bergabung di tengah sesi vote ini. Tunggu sesi berikutnya!');
        return;
    }
...
function backToCategory() {
    if (!isModerator) return;
    if (!confirm('Ganti kategori? Kartu saat ini dan sesi vote akan direset.')) return;
    roomRef.update({
        status: 'category',
...
function backToLobbyFromGame() {
    if (!isModerator) return;
    if (!confirm('Kembali ke Lobby? Sesi vote aktif akan dihapus dan semua pemain kembali ke lobby.')) return;
    roomRef.update({
        status: 'lobby',
        vote: null,
        currentCard: null,
...
function renderEdition(data) {
    ...
    if (!isModerator) {
        list.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;background:#FFF9C4;border:2px solid #F59E0B;border-radius:12px;padding:10px 14px;margin-bottom:4px;">
...
`;
    ...
    <div style="text-align:left;flex:1;">
        <div style="font-size:0.88rem;font-weight:900;color:white;text-transform:uppercase;text-shadow:1px 1px 0 rgba(0,0,0,0.4);line-height:1.2;">${edition.name}</div>
        <div style="font-size:0.7rem;font-weight:700;color:rgba(255,255,255,0.9);margin-top:3px;line-height:1.3;">${edition.desc}</div>
        <div style="font-size:0.62rem;color:rgba(255,255,255,0.85);font-weight:800;margin-top:6px;">
...
`;
    ...
    <div style="text-align:left;flex:1;">
        <div style="font-size:0.88rem;font-weight:900;color:white;text-transform:uppercase;text-shadow:1px 1px 0 rgba(0,0,0,0.4);line-height:1.2;">${cat.name}</div>
        <div style="font-size:0.7rem;font-weight:700;color:rgba(255,255,255,0.9);margin-top:3px;line-height:1.3;">${cat.desc}</div>
        <div style="margin-top:6px;background:rgba(0,0,0,0.2);border-radius:999px;height:4px;width:100%;">
            <div style="height:4px;border-radius:999px;background:rgba(255,255,255,0.9);width:${pct}%;transition:width 0.3s;"></div>
        </div>
        <div style="font-size:0.62rem;color:rgba(255,255,255,0.85);font-weight:800;margin-top:3px;">
...
function renderVotePanel(data) {
    ...
    for (let pid in players) {
        ...
        const btn = document.createElement('button');
        btn.className = 'btn-vote-player' + (isChosen ? ' voted-mine' : '');
        btn.innerHTML = `
            <span style="font-size:1.3rem;line-height:1;">${p.emoji || '👤'}</span>
            <span style="display:block;font-size:0.72rem;margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
...
function renderLeaderboard(data) {
    ...
    if (entries.every(e => e.wins === 0)) {
        list.innerHTML = '<div class="lb-empty">Belum ada ronde selesai.\n\nMainkan beberapa ronde dulu! 🎮</div>';
        return;
    }
    ...
    entries.forEach(entry => {
        ...
        row.innerHTML = `
            ...
            <div class="lb-score">
                <div class="lb-wins">${entry.wins}x 🏆</div>
            </div>
        `;
        list.appendChild(row);
    });
...
function showInfoToast(msg) {
    ...
    toast.style.cssText = `position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:#1A1A1A;color:white;padding:10px 18px;border-radius:12px;font-size:0.8rem;font-weight:700;z-index:9999;border:2px solid #FFB800;box-shadow:3px 3px 0 #FFB800;`;
...
window.onload = () => {
    ...
    if (roomFromUrl) {
        ...
        if (_saved && _saved.roomCode === roomFromUrl.toUpperCase()) {
            showInfoToast('🔄 Melanjutkan sesi sebelumnya...');
        } else {
            showInfoToast('👋 Masukkan nama untuk bergabung ke room ' + roomFromUrl.toUpperCase());
        }
    }
};
