        ...
        window.werewolfFirebaseConnected = false;
        ...
        function setFirebaseConnectionState(isConnected) {
            window.werewolfFirebaseConnected = !!isConnected;
            const label = isConnected ? 'Online' : 'Offline';
            const statusClass = isConnected ? 'is-online' : 'is-offline';
            ['firebaseHeaderStatus', 'firebaseRoomStatus'].forEach(id => {
                const badge = document.getElementById(id);
                if (!badge) return;
                badge.textContent = label;
                badge.classList.remove('is-online', 'is-offline', 'is-connecting');
                badge.classList.add(statusClass);
                badge.style.display = id === 'firebaseHeaderStatus' && !onlineRoomCode ? 'none' : 'inline-flex';
            });
            window.dispatchEvent(new CustomEvent('werewolf:firebase-connection', {
            ...
            try {
                ...
                db.ref(".info/connected").on("value", (snap) => {
                    if (snap.val() === true) {
                        ...
                        setFirebaseConnectionState(true);
                    } else {
                        console.warn("Firebase Disconnected");
                        setFirebaseConnectionState(false);
                    }
                });
        ...
        function attachRoomListeners(code) {
            ...
            timerRef.on('value', snapshot => {
                ...
                if (timerState && typeof applyTimerStateFromFirebase === 'function') {
                    applyTimerStateFromFirebase(timerState);
                }
            });
            ...
            roomPlayersRef.on('child_added', snapshot => {
                ...
                const existingPlayer = gameState.players.find(p => p.id === pId);
                if (!existingPlayer) {
                    ...
                    existingPlayer.connected = !!pd.connected;
                    existingPlayer.lastSeenAt = pd.lastSeenAt || existingPlayer.lastSeenAt || null;
                    if (pd.name) existingPlayer.name = pd.name;
                }
            });

            roomPlayersRef.on('child_changed', snapshot => {
                const pd = snapshot.val() || {};
                const pId = snapshot.key;
                const player = gameState.players.find(p => p.id === pId);
                if (!player) return;
                player.name = pd.name || player.name;
                player.connected = !!pd.connected;
                player.lastSeenAt = pd.lastSeenAt || player.lastSeenAt || null;
                renderPlayers();
                saveState();
            });
            ...
            setFirebaseConnectionState(!!window.werewolfFirebaseConnected);
        ...
        function pushAnnouncement(type, title, message, extras = {}) {
            ...
            const announcement = buildAnnouncement(type, title, message, extras);
            const updates = { announcement };
            if (announcement.event === 'morning') {
                updates.morningAnnouncement = announcement;
            }
            if (announcement.event === 'dayExecution') {
                updates.dayExecutionAnnouncement = announcement;
            }
            roomMetaRef.update(updates).catch(err => console.error('Firebase announcement error:', err));
        }
        ...
        window.addEventListener('beforeunload', (event) => {
            const isPlayerMode = !!window.WEREWOLF_PLAYER_MODE || document.body.classList.contains('mode-player');
            const hasActiveOnlineSession = !!onlineRoomCode && !isPlayerMode && (gameState.started || gameState.players.length > 0);
            if (!hasActiveOnlineSession) return;
        ...
        function pushGameStateToFirebase() {
            ...
            try {
                ...
                gameState.players.forEach(p => {
                    ...
                    updates[`${p.id}/role`] = p.role;
                    updates[`${p.id}/alive`] = p.alive;
                    updates[`${p.id}/eliminated`] = p.eliminated;
                    updates[`${p.id}/_idiotRevealed`] = !!p._idiotRevealed;
                    updates[`${p.id}/_mayorRevealed`] = !!p._mayorRevealed;
                });
        ...
        function clearOnlineVotesForCurrentDay(options = {}) {
            if (!db || !onlineRoomCode) return;
            const keepClosed = !!options.keepClosed;
            ...
            if (roomMetaRef) {
                roomMetaRef.update({
                    voteOpen: keepClosed ? false : isOnlineVotingOpenForCurrentDay(),
                    publicVoteResult: null,
                    savedVoteResult: null,
                    voteSummary: null
        ...
        function closeOnlineVotingForCurrentDay() {
            ...
            roomMetaRef.update({
                voteOpen: false,
                voteSummary: null
            }).catch(err => console.error('Firebase vote close error:', err));
        }
        ...
        const ROLE_DEFINITIONS = {
            'Wolfman': { name: 'Manusia Serigala', icon: '', category: 'wolf', team: 'Werewolf', description: 'Werewolf yang menyamar sebagai warga biasa. Ia ikut kubu serigala, tetapi terlihat bukan serigala oleh Seer, Apprentice Seer, dan Aura Seer.', count: 0, priority: 1.5 },
            ...
            // === NEUTRAL TEAM ===
            'Arsonist': { name: 'Pembakar', icon: '', category: 'neutral', team: 'Neutral', description: 'Tiap malam memilih menyiram bensin ke 1 orang, atau menyalakan api untuk membunuh semua orang yang pernah disiramnya sekaligus. Menang jika menjadi pemain terakhir yang hidup.', count: 0, priority: 18.2 },
        ...
        const ROLE_ASSET_ATTRIBUTIONS = {
            ...
            'Arsonist': 'Created by Sari Braga from Noun Project',
            ...
            'Wolfman': 'Created by iconfield from the Noun Project'
        };
        ...
        const NIGHT_ORDER = [
            ...
            'Thief', 'Troublemaker', 'Cupid', 'Mason 1', 'Mason 2', 'Doppelganger', 'Lone Wolf', 'White Wolf', 'Werewolf', 'Alpha Wolf', 'Infector', 'Seer', 'Apprentice Seer', 'Aura Seer', 'Sorcerer', 'Tracker', 'Guardian', 'Bodyguard', 'Tavern Keeper', 'Arsonist', 'Witch', 'Veteran', 'Wizard', 'Gravedigger', 'Amnesiac', 'Cult Leader', 'Vampire', 'Serial Killer', 'Little Girl', 'Sheriff', 'Martyr'
        ];
