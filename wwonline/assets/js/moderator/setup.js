...
function getAlphabeticalRoleNames() {
return Object.keys(ROLE_DEFINITIONS).sort((a, b) => a.localeCompare(b, 'id', { sensitivity: 'base' }));
}
 ...
 // ==================== TAB NAVIGATION ====================
 function refreshModeratorCommandState() {
 if (typeof gameState === 'undefined') return;

 const totalPlayers = gameState.players.length;
 const alivePlayers = gameState.players.filter(p => p.alive).length;
 const deadPlayers = Math.max(0, totalPlayers - alivePlayers);
 const selectedRoleCount = getSelectedRoleCount();
 const currentPhase = gameState.phase === 'night' ? 'Malam' : gameState.phase === 'day' ? 'Siang' : 'Setup';
 const roomCode = window.onlineRoomCode || localStorage.getItem('werewolf_online_room') || '';

 const readinessText = document.getElementById('dashboardReadinessText');
 if (readinessText) {
 if (totalPlayers < 5) readinessText.textContent = totalPlayers === 0 ? 'Belum ada pemain' : `${totalPlayers}/5 pemain minimum`;
 else if (selectedRoleCount < totalPlayers) readinessText.textContent = `Role ${selectedRoleCount}/${totalPlayers}`;
 else readinessText.textContent = gameState.started ? `${currentPhase} hari ${gameState.day}` : 'Siap memulai permainan';
 }

 const playerText = document.getElementById('dashboardPlayerText');
 if (playerText) {
 playerText.textContent = totalPlayers
 ? `${totalPlayers} pemain, ${alivePlayers} hidup, ${deadPlayers} mati`
 : '0 pemain terdaftar';
 }

 const aliveText = document.getElementById('dashboardAliveText');
 if (aliveText) {
 aliveText.textContent = totalPlayers
 ? `${alivePlayers} masih hidup, ${deadPlayers} sudah mati.`
 : 'Belum ada pemain aktif.';
 }

 const roleText = document.getElementById('dashboardRoleText');
 if (roleText) {
 if (!totalPlayers) roleText.textContent = 'Tambahkan pemain dulu';
 else if (!selectedRoleCount) roleText.textContent = 'Belum dipilih';
 else roleText.textContent = `${selectedRoleCount}/${totalPlayers} role dipilih`;
 }

 const roomText = document.getElementById('dashboardRoomText');
 if (roomText) {
 roomText.textContent = roomCode ? `Online: ${roomCode}` : 'Mode offline aktif';
 }

 const phaseHint = document.getElementById('dashboardPhaseHint');
 if (phaseHint) {
 if (!gameState.started) phaseHint.textContent = 'Setup permainan';
 else phaseHint.textContent = gameState.phase === 'day' ? `Siang hari ${gameState.day}` : `Malam ${gameState.day}`;
 }

 document.body.dataset.moderatorStarted = gameState.started ? 'true' : 'false';
 document.body.dataset.moderatorReady = totalPlayers >= 5 && selectedRoleCount >= totalPlayers ? 'true' : 'false';
 }

 window.refreshModeratorCommandState = refreshModeratorCommandState;

 function openPhaseConsole() {
 if (typeof gameState !== 'undefined' && gameState.phase === 'day') switchTab('day');
 else switchTab('night');
 }

 window.openPhaseConsole = openPhaseConsole;

	 function switchTab(tabName) {
	 if (!moderatorInitialized) initializeModeratorApp();
	 document.querySelectorAll('.tab-content').forEach(t => t.classList.remove('active'));
	 document.querySelectorAll('[data-nav], [data-nav-phase], [data-nav-more]').forEach(t => t.classList.remove('active'));
	 ...
	 if (tabName === 'night' || tabName === 'day') {
	 document.querySelectorAll('[data-nav-phase]').forEach(navBtn => navBtn.classList.add('active'));
	 } else if (!mobileNavBtn) {
	 const lainnyaBtn = document.querySelector('[data-nav-more]');
 ...
 getAlphabeticalRoleNames().forEach(role => {
...
${getAlphabeticalRoleNames().map(r => `<option value="${r}" ${player.role === r ? 'selected' : ''}>${r}</option>`).join('')}
</select>
</div>
...
`;
 ...
 if (gameState.players.length === 0) {
	 grid.innerHTML = `<div class="empty-state"><div class="empty-state-icon" data-empty-icon="users"><i data-lucide="users"></i></div><div class="empty-state-text">Belum ada pemain. Tambahkan nama lewat panel di atas.</div></div>`;
