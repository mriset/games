...
function chooseOnlineHost() {
    if (typeof verifyCreateRoomPin === 'function' && !verifyCreateRoomPin()) return;
...
function legacyPlayerToast(message) {
    ...
    toast.innerHTML = `<span>📢</span> <span>${escapePlayerHtml(message)}</span>`;
...
function playerToast(message, type = 'info', title) {
    ...
    toast.innerHTML = `
        <span class="player-toast-icon">${toastIcons[type] || toastIcons.info}</span>
...
function initPlayerListeners() {
    ...
    playerMetaRef.on('value', snapshot => {
        ...
        if (meta.gameStarted) {
            ...
            if (phaseDisplay) {
                phaseDisplay.textContent = meta.phase === 'night' ? 'MALAM HARI' : 'SIANG HARI';
                phaseDisplay.style.color = meta.phase === 'night' ? 'var(--accent-purple-light)' : 'var(--accent-gold)';
            }
            if (playerApp) {
                playerApp.classList.toggle('phase-night', meta.phase === 'night');
                playerApp.classList.toggle('phase-day', meta.phase !== 'night');
            }
    ...
    playerSelfRef.on('value', snapshot => {
        ...
        if (playerData.role && playerData.role !== myRoleData) {
            myRoleData = playerData.role;
            updatePlayerRoleCard(playerData.role);
        }
        renderPlayerVotePanel();
    });
...
<li class="player-item ${isAlive ? 'is-alive' : 'is-dead'}">
    ...
    playerTimerRef.on('value', snapshot => {
        ...
        if (!timerData || !timerData.running || timerData.secondsLeft <= 0) {
            if (timerContainer) timerContainer.style.display = 'none';
            if (timerData && timerData.secondsLeft === 0) playPlayerBuzzer();
            return;
        }

        if (timerContainer) timerContainer.style.display = 'block';
        let seconds = Math.max(0, timerData.secondsLeft - Math.floor((Date.now() - timerData.updatedAt) / 1000));
...
function renderPlayerVotePanel() {
    ...
    const isDayVote = !!(isDayPhase && meta.voteOpen !== false);
    ...
    if (!canVote) {
        status.textContent = targetId
            ...
            : 'Voting sudah dikunci oleh moderator.';
        options.innerHTML = '';
        return;
    }
...
function renderPlayerVoteReveal(result) {
    ...
    const leaderText = leaders.length > 1
        ...
        : `Vote terbanyak: ${leaders[0]?.name || result.results[0].name}`;
...
function submitPlayerVote(targetId) {
    ...
    if (playerCurrentMeta.voteOpen === false) {
        playerToast('Voting sudah dikunci moderator.', 'warning');
        return;
    }
