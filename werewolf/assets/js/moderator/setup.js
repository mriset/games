...
${roleDef ? roleIconMarkup(player.role, 'role-lucide-icon role-lucide-icon-lg') : roleIconMarkup('', 'role-lucide-icon role-lucide-icon-lg')}
</div>
<div style="font-family:'Cinzel',serif;font-size:1.3rem;font-weight:700;">${safeName}</div>
<div style="margin-top:4px;">
${player.role ? `<span class="badge badge-${roleDef.category}">${roleIconMarkup(player.role, 'role-inline-icon')} ${player.role}</span>` : '<span class="badge badge-neutral">Belum ada role</span>'}
</div>
...
${roleDef ? roleIconMarkup(p.role, 'role-lucide-icon') : roleIconMarkup('', 'role-lucide-icon')}
${p.alive ? '<div class="alive-dot"></div>' : ''}
</div>
<div class="player-info">
<div class="player-name">${safeName}</div>
<div>
${p.role ? `<span class="player-role-badge ${roleDef ? roleDef.category : 'unknown'} revealed">${roleDef ? roleIconMarkup(p.role, 'role-inline-icon') + ' ' + p.role + (p._wasDoppelganger ? ' (Eks-Doppel)' : '') : '???'}</span>` : '<span class="player-role-badge unknown">Belum ada role</span>'}
${cultBadge}
</div>
</div>
...
<div class="role-info">
${roleIconMarkup(role, 'role-lucide-icon role-dist-icon')}
