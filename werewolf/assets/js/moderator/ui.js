...
`).join('');
 ...
 { key: 'neutral', label: ' Netral', headerClass: 'team-neutral', badgeClass: 'neutral-badge', icon: '' },
 ...
 sorted.forEach(p => {
 const def = p.role ? ROLE_DEFINITIONS[p.role] : null;
 const roleLabel = def ? `${roleIconMarkup(p.role, 'role-inline-icon')} ${p.role}` : p.role || '?';
...
<span class="help-card-icon">${roleIconMarkup(role, 'role-lucide-icon')}</span>
<div>
