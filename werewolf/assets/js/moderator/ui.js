...
`).join('');
 ...
 const text = allLogs.map(l => `[${l.time}] ${l.phase === 'night' ? '' : ''} ${l.message}`).join('\n');
...
`;
 ...
 if (overlay.id === 'modalHunter' || overlay.id === 'modalWolfSacrifice' || overlay.id === 'modalConfirm' || overlay.id === 'modalCreateRoomPin') return; // Modal kritis wajib ditutup via aksi yang sah
 ...
 if (m.id === 'modalHunter' || m.id === 'modalWolfSacrifice' || m.id === 'modalConfirm' || m.id === 'modalCreateRoomPin') return; // Modal kritis wajib ditutup via aksi yang sah
