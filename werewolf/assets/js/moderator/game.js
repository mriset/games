...
${roleIconMarkup(role, 'role-lucide-icon')}
</div>`;
 ...
 title.innerHTML = `${roleIconMarkup(currentRole, 'role-inline-icon')} ${currentRole}`;
...
<span class="target-icon">${def ? roleIconMarkup(p.role, 'role-lucide-icon') : roleIconMarkup('', 'role-lucide-icon')}</span>
...
<div class="seer-result">
 ...
 <button class="btn btn-danger" onclick="veteranAlert(true)"> Mode Siaga ON</button>
 ...
 document.getElementById('nightActionResult').innerHTML = `
<div class="seer-result">
 <div class="seer-result-icon">${active ? '' : ''}</div>
...
<div style="text-align:center;padding:20px;">
 ...
 <div style="font-size:0.9rem;color:var(--text-muted);"><span class="role-emoji">${roleIconMarkup(p.role, 'role-inline-icon')}</span> Peran aslinya adalah <strong>${p.role}</strong>.</div>
</div>
`;
 hydrateWerewolfIcons(targets);
