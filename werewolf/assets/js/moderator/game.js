...
${validPlayers.map(p => {
 return `<button class="target-btn btn-sm" onclick="selectWolf1(this, '${p.id}', '${escapeJsString(p.name)}')"><span class="target-icon">${playerIconMarkup(p)}</span><span>${escapeHtml(p.name)}</span></button>`;
 }).join('')}
</div>
</div>
<div>
...
<div class="night-action-targets">
${validPlayers.map(p => {
 return `<button class="target-btn btn-sm" onclick="selectWolf2(this, '${p.id}', '${escapeJsString(p.name)}')"><span class="target-icon">${playerIconMarkup(p)}</span><span>${escapeHtml(p.name)}</span></button>`;
 }).join('')}
</div>
</div>
</div>
...
<div class="night-action-targets">
${validTargets.map(p => `
<button class="target-btn" onclick="selectCupid1(this, '${p.id}', '${escapeJsString(p.name)}')"><span class="target-icon">${playerIconMarkup(p)}</span><span>${escapeHtml(p.name)}</span></button>
`).join('')}
</div>
</div>
<div>
...
<div class="night-action-targets">
${validTargets.map(p => `
<button class="target-btn" onclick="selectCupid2(this, '${p.id}', '${escapeJsString(p.name)}')"><span class="target-icon">${playerIconMarkup(p)}</span><span>${escapeHtml(p.name)}</span></button>
`).join('')}
</div>
</div>
</div>
...
<div class="night-action-targets">
${validTargets.map(p => `
<button class="target-btn" onclick="selectTroublemaker1(this, '${p.id}', '${escapeJsString(p.name)}')"><span class="target-icon">${playerIconMarkup(p)}</span><span>${escapeHtml(p.name)}</span></button>
`).join('')}
</div>
</div>
<div>
...
<div class="night-action-targets">
${validTargets.map(p => `
<button class="target-btn" onclick="selectTroublemaker2(this, '${p.id}', '${escapeJsString(p.name)}')"><span class="target-icon">${playerIconMarkup(p)}</span><span>${escapeHtml(p.name)}</span></button>
`).join('')}
</div>
</div>
</div>
...
function processNightDeaths() {
 ...
 const deathDetails = gameState.nightDeathDetails || {};
 const detailRows = allKills
 .map(id => {
 const p = gameState.players.find(pl => pl.id === id);
 if (!p || !killNames.includes(p.name)) return '';
 const details = (deathDetails[id] || ['penyebab tidak tercatat']).join('; ');
 return `<li><strong>${escapeHtml(p.name)}</strong> <span style="color:var(--text-secondary);">(${escapeHtml(p.role || 'Tanpa Role')})</span>: ${escapeHtml(details)}</li>`;
 })
 .filter(Boolean)
 .join('');
 ...
 <div style="margin-top:8px;font-size:0.82rem;color:var(--text-secondary);">Detail moderator:</div>
 <ul style="margin:8px 0 0 18px;padding:0;font-size:0.86rem;line-height:1.55;">
 ${detailRows}
 </ul>
 `;
 ...
 'Pengumuman Pagi',
 hasDeaths
 ? `Korban malam ini: ${publicNames.join(', ')}.`
