...
function legacyPlayerToast(message) {
 ...
 toast.innerHTML = `<span class="player-toast-icon"><i data-lucide="info" aria-hidden="true"></i></span> <span>${escapePlayerHtml(message)}</span>`;
...
function updatePlayerRoleCard(role) {
 ...
 if (roleIcon) {
 roleIcon.innerHTML = role ? roleIconMarkup(role, 'role-lucide-icon role-lucide-icon-lg') : '';
